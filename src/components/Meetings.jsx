...
import { findExternalTaskOwnerName, findTaskOwnerId, meetingMinutesFromRecord, meetingTasksFromRecord, normalizeTaskDeadline } from "../meetingUtils.mjs";
...
function taskRows(tasks = [], employees = [], externalParticipantNames = []) {
...
function TaskOwnerPicker({ task, employees, externalParticipantNames, onChange }) {
  ...
  return <details className="group relative">
    ...
    <div className="absolute left-0 z-30 mt-1 max-h-64 w-72 max-w-[calc(100vw-2rem)] overflow-y-auto rounded-lg border border-slate-300 bg-white p-2 shadow-xl">
      ...
      {externalParticipantNames.length > 0 && <><div className="mb-1 mt-2 border-t border-slate-100 pt-2 text-[10px] font-bold uppercase tracking-wide text-slate-500">Účastníci mimo tým</div>{externalParticipantNames.map((name) => <label key={name} className="flex cursor-pointer items-center gap-2 rounded px-1.5 py-1.5 text-xs hover:bg-blue-50"><input type="checkbox" checked={(task.externalOwnerNames || []).includes(name)} onChange={() => toggleExternal(name)}/><span>{name}</span></label>)}</>}
    </div>
  </details>;
}
...
export default function Meetings({ employee, employees, meetings, project, onRefresh }) {
  ...
  const useAi = async () => {
    ...
    try {
      ...
      setForm((current) => ({
        ...
        tasks: taskRows(result.tasks || [], employees, form.externalParticipantNames),
      }));
  ...
  const importMeeting = async (file) => {
    ...
    try {
      ...
      setForm({
        ...
        content: result.minutes || "", tasks: taskRows(result.tasks || [], employees, externalParticipantNames),
      });
  ...
  return <div className="space-y-3">
    ...
    {view === "archive" || !isLeader ? <Card title="Porady a zápisy" subtitle={isLeader ? "Vytvořte nový zápis, nebo převeďte existující PDF, Word či fotografii do formuláře a následně do Sheetu." : "Zobrazeny jsou uložené zápisy z porad, kterých jste se účastnili nebo z nich máte přidělený úkol."} actions={isLeader ? <div className="flex flex-wrap gap-2"><input ref={importInput} className="hidden" type="file" accept=".pdf,.docx,.txt,.png,.jpg,.jpeg,.webp" onChange={(event) => importMeeting(event.target.files?.[0])}/><Button variant="secondary" disabled={busy} onClick={() => importInput.current?.click()}><Upload className="mr-1 inline" size={16}/>{busy ? "Rozpoznávám…" : "Nahrát zápis"}</Button><Button disabled={busy} onClick={() => { setForm(blankForm()); setExternalName(""); setView("form"); }}><Plus className="mr-1 inline" size={16}/>Vytvořit zápis</Button></div> : undefined}>
      {!meetingsForYear.length ? <Empty>Pro rok {selectedYear} zatím nebyl vytvořen žádný zápis.</Empty> : <div className="space-y-2">{meetingsForYear.toSorted((a,b) => b.date.localeCompare(a.date)).map((item) => <article key={item.id} className="flex flex-col gap-2 rounded-lg border border-slate-200 p-3 sm:flex-row sm:items-center sm:justify-between"><div><strong>{item.date} · Porada</strong><div className="text-sm text-slate-500">{(item.participantNames || []).join(", ")}</div></div><div className="flex flex-wrap items-center gap-2"><StatusBadge status={item.status}/><Button variant="secondary" className="min-h-8 px-2 py-1 text-xs" disabled={busy} onClick={() => setSelectedMeeting(item)}><Eye className="mr-1 inline" size={14}/>Zobrazit zápis</Button>{(isAdmin || (item.createdBy === employee.id && item.status !== "archived")) && <Button variant="secondary" className="min-h-8 px-2 py-1 text-xs" disabled={busy} onClick={() => edit(item)}><Pencil className="mr-1 inline" size={14}/>{item.status === "archived" ? "Opravit zápis" : "Upravit a dokončit"}</Button>}{item.driveFileUrl && <a className="text-sm font-bold text-blue-700 underline" href={item.driveFileUrl} target="_blank" rel="noreferrer">Otevřít PDF</a>}{isAdmin && <Button variant="danger" className="min-h-8 px-2 py-1 text-xs" disabled={busy} onClick={() => remove(item)}><Trash2 className="mr-1 inline" size={14}/>Smazat</Button>}</div></article>)}</div>}
    ...
    {selectedMeeting && <Modal title={`Zápis z porady · ${selectedMeeting.date}`} subtitle={`Zapsal/a: ${selectedMeeting.createdByName || "—"}`} className="max-w-4xl" onClose={() => setSelectedMeeting(null)}>
      <div className="space-y-3">
        ...
        <section className="rounded-lg border border-slate-200 bg-white p-4"><h3 className="mb-3 font-bold text-slate-900">Úkoly</h3>{!selectedTasks.length ? <div className="text-sm text-slate-500">Bez úkolů.</div> : <div className="overflow-x-auto"><table className="w-full min-w-[700px] text-left text-sm"><thead><tr className="border-b border-slate-200 text-xs uppercase text-slate-500"><th className="px-2 py-2">Úkol</th><th className="px-2 py-2">Odpovědné osoby</th><th className="px-2 py-2">Termín</th><th className="px-2 py-2">Stav</th></tr></thead><tbody>{selectedTasks.map((task, index) => <tr key={task.id || `${task.text}-${index}`} className="border-b border-slate-100 align-top last:border-0"><td className="px-2 py-2"><strong className="text-slate-900">{task.text}</strong>{task.status === "completed" && <div className="mt-1 rounded-md bg-emerald-50 px-2 py-1.5 text-xs text-emerald-950"><div className="whitespace-pre-wrap">{task.completionText}</div><div className="mt-1 text-[11px] text-emerald-800">Vyřídil/a {task.completedByName || "pracovník"}{task.completedAt ? ` · ${new Intl.DateTimeFormat("cs-CZ", { dateStyle: "short", timeStyle: "short" }).format(new Date(task.completedAt))}` : ""}{task.completionRecipientNames?.length ? ` · odesláno: ${task.completionRecipientNames.join(", ")}` : ""}</div></div>}</td><td className="px-2 py-2">{taskOwnerNames(task, employees).join(", ") || task.owner || "Nepřiřazeno"}</td><td className="px-2 py-2">{task.deadline || "—"}</td><td className="px-2 py-2">{task.status === "completed" ? <span className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-bold text-emerald-800">Splněno</span> : <span className="rounded-full bg-amber-100 px-2 py-1 text-xs font-bold text-amber-800">Nevyřízeno</span>}</td></tr>)}</tbody></table></div>}</section>
