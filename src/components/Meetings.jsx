...
export default function Meetings({ employee, employees, meetings, project, onRefresh }) {
  ...
  const canEditMeeting = (meeting) => canManageMeeting || !["approved", "archived"].includes(meeting.status);
  ...
  const save = async (mode) => {
    ...
    try {
      ...
      const saved = await api(endpoint, { method: form.id ? "PATCH" : "POST", body: jsonBody({ ...form, tasks: tasksForSave(), followUpTasks: tasksForSave(form.followUpTasks || []), status: scheduled ? "scheduled" : submitted ? "submitted" : "draft" }) });
      ...
      const successText = scheduled
        ...
        : submitted
          ? "Zápis čeká na schválení Odborným garantem nebo Vedoucí služby/programu."
          : "Koncept byl uložen. Kdokoli z týmu jej může doplnit a předat ke schválení.";
      resetForm(blankForm()); resetExternalName(""); setView("archive"); setNotice({ type: "success", text: `${successText}${continuityNotice}` });
      await onRefresh();
    } catch (error) { setNotice({ type: "error", text: error.message }); } finally { setBusy(false); }
  };

  const storeApprovedPdf = async (meeting) => {
    const blob = await createMeetingPdf(meeting, project);
    const body = new FormData(); body.append("file", blob, `${meeting.date}__zapis.pdf`);
    return api(`/api/meetings/${meeting.id}/pdf`, { method: "POST", body });
  };

  const approve = async (meeting) => {
    setBusy(true); setNotice(null);
    try {
      const result = await api(`/api/meetings/${meeting.id}/approve`, { method: "POST" });
      setSelectedMeeting(result.meeting);
      setNotice({ type: "success", text: "Zápis byl schválen. PDF můžete podle potřeby vytisknout nebo uložit na Disk." });
      await onRefresh();
    } catch (error) { setNotice({ type: "error", text: error.message }); } finally { setBusy(false); }
  };

  const savePdfToDrive = async (meeting) => {
    setBusy(true); setNotice(null);
    try {
      const pdf = await storeApprovedPdf(meeting);
      setSelectedMeeting(pdf.meeting);
      setNotice({ type: pdf.upload?.uploaded ? "success" : "warn", text: pdf.upload?.uploaded ? "PDF bylo uloženo na Disk." : "Uložení na Disk není nyní dostupné. PDF pro tisk lze stáhnout samostatně." });
      await onRefresh();
    } catch (error) { setNotice({ type: "error", text: error.message }); } finally { setBusy(false); }
  };

  const downloadMeetingPdf = async (meeting) => {
    setBusy(true); setNotice(null);
    try {
      const blob = await createMeetingPdf(meeting, project);
      downloadBlob(blob, `${meeting.date}__zapis_z_porady.pdf`);
      setNotice({ type: "success", text: "PDF zápisu bylo připraveno ke stažení a tisku." });
    } catch (error) { setNotice({ type: "error", text: error.message }); } finally { setBusy(false); }
  };
  ...
  return <fieldset disabled={busy} className="min-w-0 space-y-3">
    ...
    {view === "archive" ? <Card
      title="Přehled porad a zápisů"
      subtitle="Poradu může založit a zápis pořídit kterýkoli pracovník. Odeslaný zápis schvaluje Odborný garant nebo Vedoucí služby/programu."
      actions={<div className="flex flex-wrap gap-2">
        <input ref={importInput} className="hidden" type="file" accept=".pdf,.docx,.txt,.png,.jpg,.jpeg,.webp" onChange={(event) => importMeeting(event.target.files?.[0])}/>
        <Button compact variant="secondary" disabled={busy} onClick={() => importInput.current?.click()}><Upload className="mr-1 inline" size={16}/>{busy ? "Rozpoznávám…" : "Nahrát zápis"}</Button>
        <Button compact disabled={busy} onClick={startNewMeeting}><Plus className="mr-1 inline" size={16}/>Založit poradu</Button>
      </div>}
    >
      {!meetingsForYear.length ? <Empty>Pro rok {selectedYear} zatím nebyla založena žádná porada.</Empty> : <div className="space-y-2">
        {meetingsForYear.toSorted((a, b) => b.date.localeCompare(a.date)).map((item) => <article key={item.id} className="flex flex-col gap-2 rounded-lg border border-slate-200 px-3 py-2.5 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0 flex-1">
            <h3 className="text-sm font-bold">{item.date} · Porada</h3>
            <div className="mt-0.5 text-xs text-slate-500">{(item.participantNames || []).join(", ") || "Účastníci zatím neuvedeni"}{meetingMinutesAuthorName(item) ? ` · zapsal/a ${meetingMinutesAuthorName(item)}` : ""}</div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {item.status === "submitted" ? <span className="rounded-full bg-amber-100 px-2 py-1 text-xs font-bold text-amber-800">Čeká na schválení</span> : <StatusBadge status={item.status}/>}
            <Button variant="secondary" className="min-h-8 px-2 py-1 text-xs" disabled={busy} onClick={() => setSelectedMeeting(item)}><Eye className="mr-1 inline" size={14}/>{item.status === "submitted" && canManageMeeting ? "Zkontrolovat" : item.status === "scheduled" ? "Detail porady" : "Zobrazit zápis"}</Button>
            {canEditMeeting(item) && <Button variant="secondary" className="min-h-8 px-2 py-1 text-xs" disabled={busy} onClick={() => edit(item)}><Pencil className="mr-1 inline" size={14}/>{item.status === "scheduled" ? "Zapsat zápis" : ["approved", "archived"].includes(item.status) ? "Opravit zápis" : "Upravit zápis"}</Button>}
            {canManageMeeting && <Button variant="danger" className="min-h-8 px-2 py-1 text-xs" disabled={busy} onClick={() => remove(item)}><Trash2 className="mr-1 inline" size={14}/>Smazat</Button>}
          </div>
        </article>)}
      </div>}
    </Card> : <Card tone="blue" title={!form.id ? "Založit poradu" : form.originalStatus === "scheduled" ? "Zapsat zápis z porady" : ["approved", "archived"].includes(form.originalStatus) ? "Opravit zápis z porady" : "Upravit zápis z porady"} subtitle="Zápis patří do jednoho textového pole. Jednotlivé úkoly přiřaďte členům týmu nebo účastníkům mimo tým." actions={<Button variant="secondary" onClick={() => { if (!confirmUnsavedChanges()) return; resetForm(blankForm()); resetExternalName(""); setView("archive"); }}><Archive className="mr-1 inline" size={16}/>Zpět do archivu</Button>}>
      ...
      {(form.followUpTasks || []).length > 0 && <section className="mt-4 rounded-xl border border-amber-300 bg-amber-50/90 p-2.5">
        <div className="mb-2"><h3 className="font-bold text-slate-900">Nesplněné úkoly z předchozích porad</h3><p className="text-xs text-slate-600">{canManageMeeting ? "Úkoly se pouze připomínají. Změna termínu nebo odpovědné osoby upraví původní úkol a nevytvoří jeho kopii." : "Úkoly jsou pro připomenutí. Původní zadání může změnit pouze Odborný garant nebo Vedoucí služby/programu."}</p></div>
        ...
        <div className="space-y-1.5">{form.followUpTasks.map((task) => <div key={`${task.sourceMeetingId}:${task.id}`} className="grid items-end gap-1.5 rounded-md border border-amber-200 bg-white p-1.5 md:grid-cols-[minmax(0,2.2fr)_minmax(180px,1fr)_138px]">
          ...
          <div className="block"><span className="mb-0.5 block text-[11px] font-semibold text-slate-600 md:sr-only">Odpovědné osoby</span>{canManageMeeting ? <TaskOwnerPicker task={task} employees={employees} externalParticipantNames={form.externalParticipantNames} onChange={(patch) => setForm((current) => ({ ...current, followUpTasks: current.followUpTasks.map((item) => item.rowId === task.rowId ? { ...item, ...patch } : item) }))}/> : <Input className="min-h-8 bg-slate-50 py-1 text-xs" value={taskOwnerNames(task, employees).join(", ") || "Nepřiřazeno"} readOnly/>}</div>
          <label className="block"><span className="mb-0.5 block text-[11px] font-semibold text-slate-600 md:sr-only">Nový termín</span><Input className="min-h-8 px-1.5 py-1 text-xs" type="date" value={task.deadline} readOnly={!canManageMeeting} onChange={(e) => setForm((current) => ({ ...current, followUpTasks: current.followUpTasks.map((item) => item.rowId === task.rowId ? { ...item, deadline: e.target.value } : item) }))}/></label>
        </div>)}</div>
      </section>}
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {!form.id && <Button variant="secondary" disabled={busy || !form.date || hasContent} onClick={() => save("scheduled")}><Plus className="mr-1 inline" size={16}/>Založit poradu bez zápisu</Button>}
        <Button variant="secondary" disabled={busy || !hasContent} onClick={useAi}><Sparkles className="mr-1 inline" size={16}/>Uspořádat zápis pomocí Gemini</Button>
        <Button variant="secondary" disabled={busy || !form.date || !hasContent} onClick={() => save("draft")}>Uložit koncept</Button>
        <Button disabled={busy || !form.date || !hasContent} onClick={() => save("submit")}>Odeslat zápis ke schválení</Button>
      </div>
    </Card>}
    {selectedMeeting && <Modal title={`${selectedMeeting.status === "scheduled" ? "Porada" : "Zápis z porady"} · ${selectedMeeting.date}`} subtitle={meetingMinutesAuthorName(selectedMeeting) ? `Zapsal/a: ${meetingMinutesAuthorName(selectedMeeting)}` : `Založil/a: ${selectedMeeting.createdByName || "—"} · zápis dosud nebyl pořízen`} className="max-w-4xl" onClose={() => setSelectedMeeting(null)}>
      <div className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-200 bg-white p-3"><div><div className="text-xs font-bold uppercase text-slate-500">Účastníci</div><div className="mt-1 text-sm text-slate-800">{(selectedMeeting.participantNames || []).join(", ") || "Neuvedeni"}</div></div>{selectedMeeting.status === "submitted" ? <span className="rounded-full bg-amber-100 px-2 py-1 text-xs font-bold text-amber-800">Čeká na schválení</span> : <StatusBadge status={selectedMeeting.status}/>}</div>
        {selectedMeeting.approvedByName && ["approved", "archived"].includes(selectedMeeting.status) && <p className="text-xs text-emerald-800">Schválil/a: {selectedMeeting.approvedByName}{selectedMeeting.approvedAt ? ` · ${new Intl.DateTimeFormat("cs-CZ", { dateStyle: "short", timeStyle: "short" }).format(new Date(selectedMeeting.approvedAt))}` : ""}</p>}
        ...
        <div className="flex flex-wrap justify-end gap-2">
          {selectedMeeting.notes && <Button variant="secondary" onClick={() => downloadMeetingPdf(selectedMeeting)}>Stáhnout / vytisknout PDF</Button>}
          {canEditMeeting(selectedMeeting) && <Button variant="secondary" onClick={() => { const item = selectedMeeting; setSelectedMeeting(null); edit(item); }}><Pencil className="mr-1 inline" size={16}/>{selectedMeeting.status === "scheduled" ? "Zapsat zápis" : ["approved", "archived"].includes(selectedMeeting.status) ? "Opravit zápis" : "Upravit zápis"}</Button>}
          {canManageMeeting && selectedMeeting.status === "submitted" && <Button onClick={() => approve(selectedMeeting)}>Schválit zápis</Button>}
          {selectedMeeting.driveFileUrl && ["approved", "archived"].includes(selectedMeeting.status) && <a className="inline-flex min-h-9 items-center rounded-lg border border-blue-200 px-3 py-2 text-sm font-bold text-blue-700" href={selectedMeeting.driveFileUrl} target="_blank" rel="noreferrer">PDF na Disku</a>}
          {canManageMeeting && selectedMeeting.status === "approved" && !selectedMeeting.driveFileUrl && <Button variant="secondary" onClick={() => savePdfToDrive(selectedMeeting)}>Uložit PDF na Disk</Button>}
          <Button onClick={() => setSelectedMeeting(null)}>Zavřít</Button>
        </div>
      </div>
    </Modal>}
  </fieldset>;
