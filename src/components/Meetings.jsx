...
export default function Meetings({ employee, employees, meetings, project, onRefresh }) {
  ...
  const startNewMeeting = () => {
    ...
    setForm(blankForm(date, followUpsForDate(date)));
    setExternalName("");
    setNotice(null);
    setView("form");
  };
  ...
  const save = async (withPdf) => {
    ...
    try {
      ...
      setForm(blankForm()); setExternalName(""); setView("archive"); setNotice({ type: "success", text: `${withPdf ? "Zápis byl dokončen, archivován a PDF staženo pro tisk." : "Koncept byl uložen. Kdykoli jej můžete znovu otevřít a dokončit."}${continuityNotice}` });
      await onRefresh();
    } catch (error) { setNotice({ type: "error", text: error.message }); } finally { setBusy(false); }
  };
  ...
  return <div className="space-y-3">
    ...
    {view === "archive" || !isLeader ? <Card title="Porady a zápisy" subtitle={isLeader ? "Vytvořte nový zápis, nebo převeďte existující PDF, Word či fotografii do formuláře a následně do Sheetu." : "Zobrazeny jsou uložené zápisy z porad, kterých jste se účastnili nebo z nich máte přidělený úkol."} actions={isLeader ? <div className="flex flex-wrap gap-2"><input ref={importInput} className="hidden" type="file" accept=".pdf,.docx,.txt,.png,.jpg,.jpeg,.webp" onChange={(event) => importMeeting(event.target.files?.[0])}/><Button variant="secondary" disabled={busy} onClick={() => importInput.current?.click()}><Upload className="mr-1 inline" size={16}/>{busy ? "Rozpoznávám…" : "Nahrát zápis"}</Button><Button disabled={busy} onClick={startNewMeeting}><Plus className="mr-1 inline" size={16}/>Vytvořit zápis</Button></div> : undefined}>
