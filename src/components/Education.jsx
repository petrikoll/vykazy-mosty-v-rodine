...
export default function Education({
  ...
  evaluations = [], sourceEvaluation = null, readOnly = false, initialYear = new Date().getFullYear(), mode = "all", showRecordList = true,
}) {
  ...
  const recordsTable = !yearRecords.length ? <Empty>Zatím není zapsané žádné absolvované vzdělávání.</Empty> : <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead><tr className="border-b text-slate-500"><th className="p-2">Datum a čas</th><th className="p-2">Vzdělávání</th><th className="p-2">Forma</th><th className="p-2">Poskytovatel</th><th className="p-2">Plní položku plánu</th><th className="p-2">Osvědčení</th><th className="p-2 text-right">Hodiny</th></tr></thead><tbody>{yearRecords.map((item) => <tr key={item.id} className="border-b border-slate-100"><td className="whitespace-nowrap p-2">{item.dateFrom || item.date}{(item.dateTo && item.dateTo !== item.dateFrom) ? ` – ${item.dateTo}` : ""}{item.timeFrom && item.timeTo ? <span className="block text-xs text-slate-500">{item.timeFrom}–{item.timeTo}</span> : null}</td><td className="p-2 font-semibold">{item.title}</td><td className="p-2">{item.format}</td><td className="p-2">{item.provider}</td><td className="p-2">{readOnly ? (item.plannedActivityTitle || "Nepřiřazeno") : <Select disabled={busy} value={item.plannedActivityId || ""} onChange={(event) => linkRecord(item.id, event.target.value)}><option value="">Nepřiřazeno</option>{planActivityOptions.map((activity) => <option key={activity.id} value={activity.id}>{activity.topic || activity.title}</option>)}</Select>}</td><td className="p-2">{(item.driveFileId || item.localFilePath) ? <Button variant="secondary" className="min-h-8 whitespace-nowrap px-2 py-1 text-xs" disabled={busy} onClick={() => previewCertificate(item)}><Eye className="mr-1 inline" size={14}/>Otevřít</Button> : <span className="text-slate-400">—</span>}</td><td className="p-2 text-right">{item.hours}</td></tr>)}</tbody></table></div>;
  ...
  if (readOnly) {
    return <div className="space-y-3">
      <Card title="Vzdělávací plán" subtitle="Plán zapisuje a vyhodnocuje Odborný garant nebo Vedoucí služby/programu." actions={<div className="flex items-center gap-2"><Select value={year} onChange={(event) => setYear(Number(event.target.value))}>{YEARS.map((value) => <option key={value}>{value}</option>)}</Select>{current && <StatusBadge status={current.status}/>}</div>}>
      ...
      <Card title="Absolvované vzdělávání" subtitle={`Za rok ${year}: ${totalHours.toLocaleString("cs-CZ")} / 24 hodin`}>{recordsTable}</Card>
    </div>;
  }

  return <div className="space-y-3">
    <Notice notice={notice}/>
    {mode !== "records" && mode !== "evaluation" && <Card title={`Roční vzdělávací plán · ${employee.name}`} subtitle="Rozvojové potřeby a plánované vzdělávání." actions={<div className="flex items-end gap-2"><Field label="Rok"><Select value={year} onChange={(event) => setYear(Number(event.target.value))}>{YEARS.map((value) => <option key={value}>{value}</option>)}</Select></Field>{current && <StatusBadge status={current.status}/>}</div>}>
