...
export default function SignedReportUpload({ onRefresh, driveFolderUrl = "", canOpenDrive = false, title = "Hromadné nahrání podepsaných výkazů", subtitle = "Nahrajte jeden či více PDF nebo ZIP. Aplikace rozdělí stránky a navrhne přiřazení; u stejného výkazu lze spojit více stran." }) {
  ...
  const [bundles, setBundles, resetBundles] = useGuardedState([]);
  const [inputKey, setInputKey] = useState(0);
  const [analysis, setAnalysis] = useState(null);
  const [mappings, setMappings, resetMappings] = useGuardedState({});
  ...
  const commit = async () => {
    ...
    try {
      ...
      setAnalysis(null); resetBundles([]); resetMappings({}); setInputKey((value) => value + 1);
  ...
  return <Card title={title} subtitle={subtitle} actions={driveAction} collapsible>
