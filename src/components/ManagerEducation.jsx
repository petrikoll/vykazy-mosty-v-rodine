...
import { Button, Card, Empty, Field, Modal, Notice, Select, SectionTabs, StatusBadge, useTimedNotice } from "./Common.jsx";
...
export default function ManagerEducation({ portal, positions, project, onRefresh }) {
  ...
  const canDelete = ["director", "project_manager", "manager"].includes(portal.employee.appRole);
  ...
  const accessibleEmployees = useMemo(() => portal.employees
    .filter((employee) => employee.active !== false
      && employee.appRole !== "project_manager"
      && (isAdmin || employee.appRole === "worker" || employee.id === portal.employee.id))
    .sort((a, b) => a.name.localeCompare(b.name, "cs")), [portal.employee.id, portal.employees, isAdmin]);
  const [dialog, setDialog] = useState(null);
  const educationEmployees = useMemo(() => accessibleEmployees.filter(employee => isEducationEligible(employee, positions)), [accessibleEmployees, positions]);
  ...
  const rows = useMemo(() => educationEmployees.map((employee) => {
  ...
  }), [educationEmployees, portal.educationPlans, year]);
  ...
  const yearRecords = useMemo(() => portal.educationRecords
    .filter((record) => educationEmployees.some((employee) => employee.id === record.employeeId)
  ...
  [educationEmployees, portal.educationRecords, year]);
  ...
  const recordEmployee = educationEmployees.find((employee) => employee.id === recordEmployeeId) || educationEmployees[0];
  ...
  const canEditPlan = (employee) => isDirector
    || (isProjectManager && !["manager", "project_manager"].includes(employee.appRole))
    || (portal.employee.appRole === "manager" && (employee.appRole === "worker" || employee.id === portal.employee.id));
  ...
  return <div className="space-y-3">
    ...
    <SectionTabs label="Vzdělávání týmu" value={tab} onChange={setTab} items={[
      { value: "evaluations", label: "Hodnocení zaměstnanců" },
      { value: "plans", label: "Vzdělávací plány týmu" },
      { value: "records", label: "Uskutečněné vzdělávání" },
    ]}/>

    {tab === "evaluations" ? <Card title="Přehled hodnocení" subtitle="Hodnocení určí profesní cíle a rozvojové potřeby před sestavením vzdělávacího plánu." actions={<Field label="Rok"><Select value={year} onChange={(event) => { if (confirmUnsavedChanges()) setYear(Number(event.target.value)); }}>{educationYears.map((value) => <option key={value}>{value}</option>)}</Select></Field>}>
      {!evaluationRows.length ? <Empty>Nejsou založeni žádní pracovníci k hodnocení.</Empty> : <div className="overflow-x-auto"><table className="record-table w-full min-w-[860px] text-left text-sm">
        ...
        <tbody>{evaluationRows.map(({ employee, evaluation, plan }) => <tr key={employee.id} className="border-b border-slate-100"><td className="p-2"><strong>{employee.name}</strong>{employee.appRole === "manager" && <div className="text-xs text-slate-500">Odborný garant</div>}</td><td className="p-2">{evaluation?.evaluatorName || (employee.appRole === "manager" ? portal.employees.find((item) => item.appRole === "director")?.name : portal.employees.find((item) => item.appRole === "manager")?.name) || "—"}</td><td className="p-2">{evaluation?.evaluationDate || "—"}</td><td className="p-2"><StatusBadge status={evaluation?.status || "missing_evaluation"}/></td><td className="p-2">{plan ? <StatusBadge status={plan.status}/> : <span className="text-slate-400">Nevytvořen</span>}</td><td className="p-2"><div className="flex min-w-max justify-end gap-1.5"><Button compact variant="secondary" className="whitespace-nowrap" disabled={!evaluation && !canEditEvaluation(employee)} onClick={() => openDialog(employee.id, "employeeEvaluation")}><Eye className="mr-1 inline" size={16}/>{evaluation ? "Otevřít hodnocení" : "Zahájit hodnocení"}</Button>{canDelete && evaluation && <Button compact variant="danger" className="whitespace-nowrap" disabled={busy} onClick={() => deleteEvaluation(evaluation, employee)}><Trash2 className="mr-1 inline" size={16}/>Smazat</Button>}</div></td></tr>)}</tbody>
      </table></div>}
    </Card> : tab === "plans" ? <>
      <Card title="Přehled vzdělávacích plánů" subtitle="Jeden řádek za pracovníka; podrobnosti se otevřou až po výběru." actions={<Field label="Rok"><Select value={year} onChange={(event) => { if (confirmUnsavedChanges()) setYear(Number(event.target.value)); }}>{educationYears.map((value) => <option key={value}>{value}</option>)}</Select></Field>}>
        {!rows.length ? <Empty>Nejsou založeni žádní pracovníci.</Empty> : <div className="overflow-x-auto"><table className="record-table w-full min-w-[780px] text-left text-sm">
          ...
          <tbody>{rows.map((row) => <tr key={row.employee.id} className={`border-b border-slate-100 ${dialog?.employeeId === row.employee.id ? "bg-blue-50/60" : ""}`}><td className="p-2"><strong>{row.employee.name}</strong>{row.employee.appRole === "manager" && <div className="text-xs text-slate-500">Odborný garant</div>}{row.employee.appRole === "director" && <div className="text-xs text-slate-500">Vedoucí služby/programu</div>}</td><td className="p-2"><StatusBadge status={row.plan?.status || "missing_plan"}/></td><td className="p-2 text-right font-semibold">{row.plan ? `${formatHours(row.plannedHours)} h` : "—"}</td><td className="p-2 text-right font-semibold">{row.plan ? formatMoney(row.plannedCost) : "—"}</td><td className="p-2">{row.plan?.evaluation ? <span className="font-semibold text-emerald-700">Vyplněno</span> : <span className="text-slate-500">Nevyplněno</span>}</td><td className="p-2"><div className="flex min-w-max justify-end gap-1.5"><Button compact variant="secondary" className="whitespace-nowrap" onClick={() => openDialog(row.employee.id, "plan")}><Eye className="mr-1 inline" size={16}/>Otevřít plán</Button><Button compact variant="secondary" className="whitespace-nowrap" disabled={!row.plan} onClick={() => openDialog(row.employee.id, "annualEvaluation")}><ClipboardCheck className="mr-1 inline" size={16}/>Roční vyhodnocení</Button>{canDelete && row.plan && <Button compact variant="danger" className="whitespace-nowrap" disabled={busy} onClick={() => deletePlan(row.plan, row.employee)}><Trash2 className="mr-1 inline" size={16}/>Smazat plán</Button>}</div></td></tr>)}</tbody>
          {isAdmin && <tfoot className="border-t-2 bg-violet-50 font-bold text-violet-900"><tr><td className="p-2" colSpan="3">Schválené plány celkem</td><td className="p-2 text-right">{formatMoney(approvedCost)}</td><td colSpan="2"/></tr></tfoot>}
        </table></div>}
      </Card>
    </> : <>
      <Card title="Evidence absolvovaného vzdělávání" subtitle="Vzdělávání členů RT v přímé práci na pracovní smlouvu (HPP)." actions={<div className="flex flex-wrap items-end gap-2"><Field label="Rok"><Select value={year} onChange={(event) => { if (confirmUnsavedChanges()) setYear(Number(event.target.value)); }}>{educationYears.map((value) => <option key={value}>{value}</option>)}</Select></Field></div>}>
        {!yearRecords.length ? <Empty>Pro rok {year} zatím není zapsané žádné uskutečněné vzdělávání.</Empty> : <div className="overflow-x-auto"><table className="record-table w-full min-w-[960px] text-left text-sm"><thead><tr className="border-b text-slate-500"><th className="p-2">Pracovník</th><th className="p-2">Datum a čas</th><th className="p-2">Vzdělávání</th><th className="p-2">Plní položku plánu</th><th className="p-2">Osvědčení</th><th className="p-2 text-right">Hodiny</th>{canDelete && <th className="p-2 text-right">Akce</th>}</tr></thead><tbody>{yearRecords.map((record) => <tr key={record.id} className="border-b border-slate-100"><td className="p-2 font-semibold">{record.employeeName}</td><td className="whitespace-nowrap p-2">{record.dateFrom || record.date}{record.dateTo && record.dateTo !== record.dateFrom ? ` – ${record.dateTo}` : ""}{record.timeFrom && record.timeTo && <span className="block text-xs text-slate-500">{record.timeFrom}–{record.timeTo}</span>}</td><td className="p-2"><strong>{record.title}</strong><span className="mt-0.5 block text-xs text-slate-500">{[record.provider, record.format].filter(Boolean).join(" · ")}</span></td><td className="p-2"><Select aria-label={`Položka plánu pro ${record.employeeName}: ${record.title}`} className="max-w-52" disabled={busy} value={record.plannedActivityId || ""} onChange={(event) => linkRecord(record.id, event.target.value)}><option value="">Nepřiřazeno</option>{planActivitiesFor(record).map((activity) => <option key={activity.id} value={activity.id}>{activity.topic || activity.title}</option>)}</Select></td><td className="p-2">{(record.driveFileId || record.localFilePath) ? <Button variant="secondary" className="min-h-8 whitespace-nowrap px-2 py-1 text-xs" disabled={busy} onClick={() => previewCertificate(record)}><Eye className="mr-1 inline" size={14}/>Otevřít</Button> : <span className="text-slate-400">—</span>}</td><td className="p-2 text-right">{formatHours(record.hours)}</td>{canDelete && <td className="p-2 text-right"><Button variant="danger" className="min-h-8 px-2 py-1 text-xs" disabled={busy} onClick={() => deleteEducationRecord(record)}><Trash2 className="mr-1 inline" size={14}/>Smazat</Button></td>}</tr>)}</tbody><tfoot className="border-t-2 bg-slate-50 font-bold"><tr><td className="p-2" colSpan="5">Celkem za tým</td><td className="p-2 text-right">{formatHours(totalRecordedHours)} h</td>{canDelete && <td/>}</tr></tfoot></table></div>}
      </Card>
      {recordEmployee && <Card title="Zapsat absolvované vzdělávání" tone="blue" collapsible>
        <div className="mb-3 max-w-sm"><Field label="Pracovník"><Select value={recordEmployee.id} onChange={(event) => { if (confirmUnsavedChanges()) setRecordEmployeeId(event.target.value); }}>{educationEmployees.map((employee) => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</Select></Field></div>
        <Education key={`records-${recordEmployee.id}-${year}`} employee={recordEmployee} actor={portal.employee} employees={portal.employees} positions={positions} project={project} plans={employeePlans(recordEmployee)} records={employeeRecords(recordEmployee)} onRefresh={onRefresh} initialYear={year} mode="records" showRecordList={false} embedded/>
      </Card>}
    </>}
    {selectedEmployee && dialog && <Modal title={`${dialog.view === "employeeEvaluation" ? "Hodnocení zaměstnance" : dialog.view === "annualEvaluation" ? "Roční vyhodnocení" : "Vzdělávací plán"} · ${selectedEmployee.name}`} subtitle={`Rok ${year}`} onClose={() => setDialog(null)}>
      {dialog.view === "employeeEvaluation" ? <EmployeeEvaluation
        ...
        onOpenPlan={isEducationEligible(selectedEmployee, positions) ? () => setDialog({ employeeId: selectedEmployee.id, view: "plan" }) : undefined}
      /> : <Education key={`${selectedEmployee.id}-${year}-${dialog.view}`} employee={selectedEmployee} actor={portal.employee} employees={portal.employees} positions={positions} project={project} plans={employeePlans(selectedEmployee)} records={employeeRecords(selectedEmployee)} evaluations={employeeEvaluations(selectedEmployee)} sourceEvaluation={evaluationFor(selectedEmployee)} onRefresh={onRefresh} initialYear={year} mode={dialog.view === "annualEvaluation" ? "evaluation" : dialog.view} readOnly={!canEditPlan(selectedEmployee)} embedded/>}
    </Modal>}
  </div>;
}
